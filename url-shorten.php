 ...
 */
function smswp_filter_message_urls( $message, $args ) {
	global $bitly_token, $group_guid;
	if( $args['url_shorten_bitly'] && $args['url_shorten_bitly_token'] ) {
		$bitly_token = apply_filters( 'smswp_bitly_token', $args['url_shorten_bitly_token'] );
		$group_guid = smswp_get_bitly_group_guid( $bitly_token );
		if( $group_guid ) {
			$regex = '"\b(https?://\S+)"';
			$message = preg_replace_callback( $regex, function( $url ) {
				return smswp_url_shorten_bitly( $url[0] );
			}, $message );
		}
	...
	if( $args['url_shorten'] && $args['url_shorten_api_key'] ) {
		$regex = '"\b(https?://\S+)"';
		$message = preg_replace_callback( $regex, function( $url ) {
			return smswp_url_shorten_google( $url[0] );
		}, $message );
	}
...
add_filter( 'smswp_sms_message', 'smswp_filter_message_urls', 9999, 2 );
 ...
 */
function smswp_url_shorten_google( $url ) {
	$options = smswp_get_options();

	$result = wp_remote_post( add_query_arg( 'key', apply_filters( 'smswp_google_api_key', $options['url_shorten_api_key'] ), 'https://www.googleapis.com/urlshortener/v1/url' ), array(
 ...
 */
function smswp_url_shorten_bitly( $url ) {
	global $bitly_token, $group_guid;
...
function smswp_get_bitly_group_guid( $bitly_token ) {
	
	$group_guid = get_transient( 'smswp_bitly_group_guid' );
	if( !$group_guid ) {
		...
		if ( !is_wp_error( $result ) ) {
			...
			if( isset( $result->groups ) && is_array( $result->groups ) ) {
				...
				if( isset( $group->guid ) ) {
					...
					set_transient( 'smswp_bitly_group_guid', $group_guid, WEEK_IN_SECONDS );
				}
			}
		}
	}
